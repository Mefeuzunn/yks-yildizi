...
export async function GET(req: Request) {
  try {
    ...
    const plans = await db.prepare('SELECT * FROM study_plans WHERE user_id = ?').all(sessionId);
...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'add') {
      ...
      ).get(sessionId, day, time, duration, title, notes || '', color || '#3b82f6', 0) as any;
    ...
    if (action === 'update') {
      ...
      ).run(day, time, duration, title, notes, color, completed, id, sessionId);
      return NextResponse.json({ success: true });
    }

    if (action === 'delete') {
      await db.prepare('DELETE FROM study_plans WHERE id = ? AND user_id = ?').run(id, sessionId);
      return NextResponse.json({ success: true });
    }
